function [sumPerformanceCountsByClass] = subjects_crossValidation(showPlots, savePlots, window_size, hiddenLayers, performanceCountsTolerance, data_columns)
...
% Leave-one-out cross validation by subject
for i = 1 : length(data)
    ...
    if savePlots
        ...
        print( strcat('./Graphs/SubjectCrossVal/',titles{i},'L'), '-dpdf')
    ...
    if savePlots
        ...
        set(gcf,'PaperOrientation','landscape');
        set(gcf,'Position',[50 50 1200 800]);
        print( strcat('./Graphs/SubjectCrossVal/',titles{i},'R'), '-dpdf')
        if closePlotAfterSaving
            close
        end
    end
